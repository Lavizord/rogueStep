...
const useBackpack = () => {
    ...
    // TODO: substituir addItem() pelo handleAddBackpack
    /*
    ...
    const handleAddBackpackByItemList = (items: Item[]) => {
        ...
        items.forEach((newItem: Item) => {
           ...
           if(newItem.stackable){
                ...
                if(!isUndefined(currentItem)){                  // Se existir adicionamos a quantidade ao existente.
                    currentItem.quantity += newItem.quantity
                } else if(!isFull){                             // Se não existir vemos se estamos no limite antes de adicionar.
